...
void my_aes_create_key(const unsigned char *key, unsigned int key_length,
                       uint8 *rkey)
{
...
int main(void)
{
  ...
  const unsigned char key[] = "Hotpu";
  unsigned char data[64] = "root";
  // memcpy((void*)plain,data,32);
  unsigned char rkey[32];
  my_aes_create_key(key, 5, rkey);
  printf("sc_aes_ecb rkey: \n");
  for(i = 0; i < 16; i++)
  {
    printf("%02x ",rkey[i]);
  }
  printf(("\n"));
  aes_encrypt_ecb(rkey,16,data,cipher,1);
  ...
  printf(("\n"));   
  if(aes_decrypt_ecb(rkey,16,cipher,plain,1) == 0x0)
  {
